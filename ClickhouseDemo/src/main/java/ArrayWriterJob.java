...
public class ArrayWriterJob {
    public static void main(String[] args) {
        String clickhouseUrl = "jdbc:clickhouse://localhost:8123";
        ...
        try (ClickHouseConnection connection = dataSource.getConnection()) {
            ...
            sth.write().send("INSERT INTO test.array_test", new ClickHouseStreamCallback() {
                @Override
                public void writeTo(ClickHouseRowBinaryStream stream) throws IOException {
                    for (int rowCount = 1; rowCount <= 200000; rowCount++) {
